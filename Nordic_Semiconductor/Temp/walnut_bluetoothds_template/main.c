...
#define APP_ADV_FAST_INTERVAL            MSEC_TO_UNITS(60, UNIT_0_625_MS)
#define APP_ADV_FAST_TIMEOUT             30                                         // Should be 30s according to BT spec
#define APP_ADV_SLOW_INTERVAL            MSEC_TO_UNITS(1200, UNIT_0_625_MS)
#define APP_ADV_SLOW_TIMEOUT             0
#define APP_ADV_BONDED_FAST_INTERVAL     MSEC_TO_UNITS(60, UNIT_0_625_MS)
#define APP_ADV_BONDED_FAST_TIMEOUT      10                                         // Should be 30s according to BT spec
#define APP_ADV_BONDED_SLOW_INTERVAL     MSEC_TO_UNITS(1200, UNIT_0_625_MS)
#define APP_ADV_BONDED_SLOW_TIMEOUT      20
...
static void advertising_init(bool bonded);
static bool is_bonded(void);
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            if (!m_bonded && is_bonded()) {
                advertising_stop();
                m_bonded = true;
                advertising_init(m_bonded);
...
static void advertising_init(bool bonded)
{
    ...
    if (m_bonded) {
        ...
        options.ble_adv_fast_interval     = APP_ADV_BONDED_FAST_INTERVAL;
        options.ble_adv_fast_timeout      = APP_ADV_BONDED_FAST_TIMEOUT;
        options.ble_adv_slow_enabled      = BLE_ADV_SLOW_ENABLED;
        options.ble_adv_slow_interval     = APP_ADV_BONDED_SLOW_INTERVAL;
        options.ble_adv_slow_timeout      = APP_ADV_BONDED_SLOW_TIMEOUT;        
    } else {
        options.ble_adv_fast_enabled      = BLE_ADV_FAST_ENABLED;
...
static void sensors_callback(sensors_evt_t evt, sensors_data_t *data)
{
    switch (evt) {
        case SI7020_TEMP_MEAS_EVT:
            APPL_LOG("[APPL] Temperature measurement.\r\n");
            bluetooth_update_temperature(data->temperature);
            ble_advertising_start(BLE_ADV_MODE_DIRECTED);
            break;
        case SI7020_HUM_MEAS_EVT:
            APPL_LOG("[APPL] Humidity measurement.\r\n");
            bluetooth_update_temperature(data->temperature);

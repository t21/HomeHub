...
uint8_t BleAdvertisingParser::getDeviceIndex(char *str, uint8_t strLen) {
    char tempStr[3];
    ...
    uint8_t deviceIndex = (uint8_t)strtol(tempStr, NULL, 10);
...
uint8_t BleAdvertisingParser::getSensorDataLength(char *str, uint8_t strLen) {
    char tempStr[3];
    ...
    uint8_t sensorDataLen = (uint8_t)strtol(tempStr, NULL, 16);
    
    #ifdef PRINT_DEBUG_MESSAGES
        Serial.println(sensorDataLen);
    #endif
...
uint8_t BleAdvertisingParser::getBatteryCapacity(char *str, uint8_t strLen) {
    char tempStr[3];
    ...
    uint8_t batteryCapacity = (uint8_t)strtol(tempStr, NULL, 16);
...
//    advDataLen = (int)strtol(tempStr, NULL, 16);
    ...
    if (sensorId == SENSOR_ID_BATTERY_CAPACITY) {
        return getBatteryCapacity(str, strLen);
    }
    
    advDataLen = getSensorDataLength(str, strLen);
...
//        for (int i = offset; i < str.length(); i+=2) {
        ...
        while (index < advDataLen) {
            ...
            sId = (advData[index++] << 8) | advData[index++];
                    
            switch (sId) {
                case SENSOR_ID_TEMPERATURE:
                    sVal = (int16_t)((advData[index++] << 8) | advData[index++]) / 10.0;
                    if (sId == sensorId) {
                        #ifdef PRINT_DEBUG_MESSAGES
                ...
                case SENSOR_ID_HUMIDITY:
                    sVal = (uint16_t)((advData[index++] << 8) | advData[index++]) / 10.0;
                    if (sId == sensorId) {
                        #ifdef PRINT_DEBUG_MESSAGES
                            Serial.print("Sensor id:"); Serial.println(sId);
                            Serial.print("Sensor value:"); Serial.println(sVal);
                ...
                case SENSOR_ID_AMBIENT_LIGHT:
                    sVal = (uint16_t)((advData[index++] << 8) | advData[index++]);
                    if (sId == sensorId) {
                        #ifdef PRINT_DEBUG_MESSAGES
                            Serial.print("Sensor id:"); Serial.println(sId);
                            Serial.print("Sensor value:"); Serial.println(sVal);
